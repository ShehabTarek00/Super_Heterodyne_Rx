...
% Add a vertical line at f=0
hold on;
xline(0, 'k--'); % 'k--' specifies red and dashed line style
xline(100e3, 'k--');
xline(150e3, 'k--');
xline(200e3, 'k--');
xline(250e3, 'k--');
...
% Definging indeces for each message to choose its FPass frequencies easily
...
LO_Osc = cos(2 * pi * (Carrier_Frequencies(Chosen_Message)+W_IF+ 1200) * T);
...
audiowrite('output Freq Shift 1.2KHz Osc QuranPalestine .wav',BB_Msg_Resampled_44KHz, Fs);
%-------------------------------------------------------------------------
